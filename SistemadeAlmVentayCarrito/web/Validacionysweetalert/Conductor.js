...
function validarnewconductor() {
    debugger
    var nombre = document.frm_nuevo.txtchofer.value;
    var tipodoc = document.frm_nuevo.Txtidtipodocumento.value;
    var documento = document.frm_nuevo.Txtnumerodocumento.value;
    var licen = document.frm_nuevo.Txtlicencia.value;
...
function validareditconductor() {
    var nombre = document.frm_edit.txtchofer.value;
    var tipodoc = document.frm_edit.Txtidtipodocumento.value;
    var documento = document.frm_edit.Txtnumerodocumento.value;
    var Direccion = document.frm_edit.Txtdireccion.value;
    var Licencia = document.frm_edit.Txtlicencia.value;
    if (tipodoc === "4") {
        if (documento.length !== 11) {
            debugger;
            swal("falta caracteres  o demasiados caracteres RUC", {
...
$(function () {
    $("#newconductor").on("submit", function (e) {
        e.preventDefault();
        var data = $('#newconductor').serialize();
        $.post("ConductoresController?accion=add", data, function (res, est, jqXHR) {
            if (res === "ok") {
                swal("Se ha guardado la informacion", {
                    icon: "success"
                        ...
                        .then((willDelete) => {
                            if (willDelete) {
                                parent.location.href = "Conductor.jsp";
                            }
                        });
                ;
            }else if (res === "yaexiste") {
                debugger;
                swal("El chofer ya esta registrado", {
                    icon: "warning"
                });
                return false;
            } else {
...
$(function () {
    $("#editconductor").on("submit", function (e) {
        e.preventDefault();
        var data = $('#editconductor').serialize();
        $.post("ConductoresController?accion=Actualizar", data, function (res, est, jqXHR) {
            if (res === "ok") {
                swal("Se ha guardado la informacion", {
                    icon: "success"
                })
                ...
                swal("Hubo un error", "Intente nuevamente", {
                    icon: "warning"
                        ...
                        .then((willDelete) => {
                            if (willDelete) {
                                parent.location.href = "EditarConductor.jsp";
                            }
            ...
            dangerMode: true
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        ...
                        console.log(fila);
                        var idcon = fila.find('#idcond').text();
                        console.log(idcon);
                        var data = {"accion": "eliminar", idCon: idcon};
                        $.post("ConductoresController", data, function (res, est, jqXHR) {
                            //console.log('res ',res);
            ...
            dangerMode: true
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        ...
                        var fila = $(this).parent().parent();
                        console.log(fila);
                        var idcat = fila.find('#idcond').text();
                        console.log(idcat);
