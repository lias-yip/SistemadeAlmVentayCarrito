 ...
 * @author yenny
 */
public class RefCotiaPedidoventaController extends HttpServlet {

    MovimientoDAO fvdao = new MovimientoDAO();
    int id;
    int idref;
    MovimientoDAO fvDAO;
    // venta
    ...
    public RefCotiaPedidoventaController() {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (accion.equalsIgnoreCase("procesarCarritorefpe")) {
            this.procesarCarritorefpe(request, response);
        }
        if (accion.equalsIgnoreCase("refPe")) {
            Detallerefpe(request, response);
        }
        if (accion.equalsIgnoreCase("refpeActualizarcantidad")) {
            refpeActualizarcantidad(request, response);
        }
        if (accion.equals("actualizarcostofv")) {
            this.actualizarcostofv(request, response);
        }
        if (accion.equals("AnadirCarritorefpe")) {
            this.anadirProductorefpe(request, response);
        }
        if (accion.equals("RegistrarrefPedido")) {
            this.Registrarrefpedido(request, response);
        }
    ...
    private void procesarCarritorefpe(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        HttpSession sesion = request.getSession();
        ArrayList<DetalleMovimiento> movi;//
        if (sesion.getAttribute("PedidVenta") == null) {
            movi = new ArrayList<DetalleMovimiento>();

        } else {
            movi = (ArrayList<DetalleMovimiento>) sesion.getAttribute("PedidVenta");
        }
        ...
        response.sendRedirect("RefInsertarPedidoVenta.jsp");

    }

    private void anadirProductorefpe(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        ...
        if (sesion.getAttribute("PedidVenta") == null) {
            ...
            carrito = (ArrayList<DetalleMovimiento>) sesion.getAttribute("PedidVenta");
        }
        ...
        sesion.setAttribute("PedidVenta", carrito);
        request.getRequestDispatcher("RefInsertarPedidoVenta.jsp").forward(
                request, response);
    }

    protected void refpeActualizarcantidad(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String acceso = "RefInsertarPedidoVenta.jsp";
    ...
    private void actualizarcostofv(HttpServletRequest request, HttpServletResponse response)
        ...
        if (sesion.getAttribute("PedidVenta") == null) {
            ...
            caroc = (ArrayList<DetalleMovimiento>) sesion.getAttribute("PedidVenta");
        }
        ...
        det.setCosto(Costo);
        caroc.set(fila, det);
        sesion.setAttribute("PedidVenta", caroc);
        response.sendRedirect("RefInsertarPedidoVenta.jsp");

    }

    protected void Detallerefpe(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        int id;
    ...
    public ArrayList<DetalleMovimiento> ObtenerSesion(HttpServletRequest request) {
        ...
        if (request.getSession().getAttribute("PedidVenta") == null) {
            lista = new ArrayList<>();
        } else {
            lista = (ArrayList<DetalleMovimiento>) request.getSession().getAttribute("PedidVenta");
        }
        return lista;
    }

    public void GuardarSesion(HttpServletRequest request, List<DetalleMovimiento> lista) {
        request.getSession().setAttribute("PedidVenta", lista);
    }

    private void Registrarrefpedido(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        HttpSession sesion = request.getSession();
