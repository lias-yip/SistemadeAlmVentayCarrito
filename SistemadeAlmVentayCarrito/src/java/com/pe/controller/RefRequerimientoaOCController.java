 ...
 * @author yenny
 */
public class RefRequerimientoaOCController extends HttpServlet {
    ...
    public RefRequerimientoaOCController() {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (accion.equals("RegistraroOrdendecompra")) {
            this.RegistraroOrdendecompra(request, response);
        }
        if (accion.equalsIgnoreCase("procesarCarritorefreq")) {
            this.procesarCarritorefreq(request, response);
        }
        if (accion.equalsIgnoreCase("refreq")) {
            Detallerefreq(request, response);
        }
        if (accion.equalsIgnoreCase("refreqActualizarcantidad")) {
            refreqActualizarcantidad(request, response);
        }
          if (accion.equals("actualizarcostorefOC")) {
            this.actualizarcostorefOC(request, response);
        }
        if (accion.equals("AnadirCarritorefoc")) {
            this.anadirProductorefoc(request, response);
        }
    }

    private void procesarCarritorefreq(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        HttpSession sesion = request.getSession();
        ArrayList<DetalleMovimiento> movi;//
        if (sesion.getAttribute("RefOrdendecompra") == null) {
            movi = new ArrayList<DetalleMovimiento>();

        } else {
            movi = (ArrayList<DetalleMovimiento>) sesion.getAttribute("RefOrdendecompra");
        }
    ...
    private void anadirProductorefoc(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        ...
        if (sesion.getAttribute("RefOrdendecompra") == null) {
            ...
            carrito = (ArrayList<DetalleMovimiento>) sesion.getAttribute("RefOrdendecompra");
        }
        ...
        sesion.setAttribute("RefOrdendecompra", carrito);
        request.getRequestDispatcher("RefInsertarOrdencompradesdeRQ.jsp").forward(
                request, response);
    }

    protected void refreqActualizarcantidad(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String acceso = "RefInsertarOrdencompradesdeRQ.jsp";
        MovimientoDAO pdao = new MovimientoDAO();
        List<DetalleMovimiento> listS = ObtenerSesion(request);
        //PROCEDIMIENTO ACTUALIZAR
        double cantidad = Double.parseDouble(request.getParameter("cantgrref"));
    ...
    private void actualizarcostorefOC(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        HttpSession sesion = request.getSession();
        ...
        if (sesion.getAttribute("RefOrdendecompra") == null) {
            ...
            caroc = (ArrayList<DetalleMovimiento>) sesion.getAttribute("RefOrdendecompra");
        }
        Double Costo = Double.parseDouble(request.getParameter("costorefOC"));
        int fila = Integer.parseInt(request.getParameter("idproductorefOC"));
        DetalleMovimiento det = caroc.get(fila);
        det.setCosto(Costo);
        caroc.set(fila, det);
        sesion.setAttribute("RefOrdendecompra", caroc);
        response.sendRedirect("RefInsertarOrdencompradesdeRQ.jsp");

    }
    protected void Detallerefreq(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        int id;
        if (request.getParameter("id") == null) {
            id = Integer.parseInt(request.getSession().getAttribute("NotaSa").toString());
        } else {
        ...
        GuardarSesion(request, listS);
        request.getSession().setAttribute("NotaSa", id);
        RequestDispatcher vista = request.getRequestDispatcher(acceso);
        vista.forward(request, response);
    }
    ...
    public ArrayList<DetalleMovimiento> ObtenerSesion(HttpServletRequest request) {
        ...
        if (request.getSession().getAttribute("RefOrdendecompra") == null) {
            lista = new ArrayList<>();
        } else {
            lista = (ArrayList<DetalleMovimiento>) request.getSession().getAttribute("RefOrdendecompra");
        }
        return lista;
    }

    public void GuardarSesion(HttpServletRequest request, List<DetalleMovimiento> lista) {
        request.getSession().setAttribute("RefOrdendecompra", lista);
    }

    private void RegistraroOrdendecompra(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        HttpSession sesion = request.getSession();
        ...
        if (niDAO.insertarMovimiento(v, detalle)) {
            request.getSession().removeAttribute("proveedorROC");
