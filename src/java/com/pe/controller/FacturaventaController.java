 ...
 * @author yenny
 */
public class FacturaventaController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (accion.equalsIgnoreCase("procesarCarritofb")) {
            this.procesarCarritofb(request, response);
        }
        if (accion.equals("AnadirCarritofb")) {
            this.anadirCarritofb(request, response);
        }
        if (accion.equals("actualizarcantidadfb")) {
            this.actualizarcantidadfb(request, response);
        }
     if (accion.equals("actualizarcostofb")) {
            this.actualizarcostofb(request, response);
       }
        if (accion.equals("Registrarfacturaboleta")) {
            this.Registrarfacturaboleta(request, response);
        }
        ...
        if (accion.equals("Limpiarfb")) {
            this.Limpiarfb(request, response);
        }
    }

    private void procesarCarritofb(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        HttpSession sesion = request.getSession();
        ArrayList<DetalleMovimiento> carfb;//
        if (sesion.getAttribute("carfb") == null) {
            carfb = new ArrayList<DetalleMovimiento>();

        } else {
            carfb = (ArrayList<DetalleMovimiento>) sesion.getAttribute("carfb");
        }
        if (carfb.size() > 0) {
            for (int i = 0; i < carfb.size(); i++) {

                DetalleMovimiento det = carfb.get(i);
        ...
        sesion.setAttribute("carfb", carfb);
        response.sendRedirect("InsertarFacturaventa.jsp");
    }

    private void anadirCarritofb(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        double subtotal;
        HttpSession sesion = request.getSession();//
        ArrayList<DetalleMovimiento> carfb;//
    ...
    private void actualizarcostofb(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        HttpSession sesion = request.getSession();
        ArrayList<DetalleMovimiento> carfb;//
        if (sesion.getAttribute("carfb") == null) {
        ...
        Double Costo = Double.parseDouble(request.getParameter("costfb"));
        int fila = Integer.parseInt(request.getParameter("idproductofb"));
    ...
    private void actualizarcantidadfb(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        HttpSession sesion = request.getSession();
        ArrayList<DetalleMovimiento> carfb;//
        if (sesion.getAttribute("carfb") == null) {
        ...
        Double cantidad = Double.parseDouble(request.getParameter("cantfb"));
        int fila = Integer.parseInt(request.getParameter("idproductofb"));
        DetalleMovimiento det = carfb.get(fila);
    ...
    private void Registrarfacturaboleta(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        HttpSession sesion = request.getSession();
        ...
        ArrayList<DetalleMovimiento> detalle = (ArrayList<DetalleMovimiento>) sesion.getAttribute("carfb");
        if (fbDAO.insertarMovimiento(v, detalle) && fbDAO.addinsersalida(v, detalle)) {
            request.getSession().removeAttribute("clienteFB");
            request.getSession().removeAttribute("carfb");
    ...
    private void estadoanularfbxx(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
    ...
    private void Anular(HttpServletRequest request, HttpServletResponse response)
        ...
        fb.setEstado("Pendiente");
        fbDAO.editarEstadoref(fb, idref);
    ...
    private void Limpiarfb(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        request.getSession().removeAttribute("clienteFB");
        request.getSession().removeAttribute("carfb");
        response.sendRedirect("InsertarFacturaventa.jsp");
